...
### 14.5 工具集增量演进 —— 数量是快照，不是不变量

- MCP 工具集随业务需要动态增加。「当前有几个工具」是**描述性快照**（住
  `test/mcp_tools/spec.md §1 总账`），**不是写死的不变量**。
- 新增工具 = `tools/__init__.py::ALL_TOOLS` 加一条 + spec §1 总账 +1 行 + §6 加
  status_code 映射即可：
  - **不为「多一个工具」开 ADR**（仅当工具引入新架构决策才开 ADR）
  - **不在测试里 assert 工具总数**：`len(ALL_TOOLS)==N` 这种写死断言禁止；
    用工具名白名单 set 校验「含且仅含哪些工具」，数量由白名单隐含
  - 文档措辞不写死固定数字（「N 个工具是完整对外面」是反模式）
- 同源 CLAUDE.md §8 反模式「不在长期约束里写当前清单（当前 N 个…）」。
...
- v6 2026-06-13 追加 §14.5 工具集增量演进规则：
  - 工具数量是描述性快照（住 spec §1 总账），不是写死的不变量
  - 新增工具不开 ADR（除非引入新架构决策）、不在测试 assert 总数、文档不写死固定数字
  - 同源 CLAUDE.md §8 反模式「不在长期约束里写当前清单」
