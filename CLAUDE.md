...
**实现 + 测试 + 验收通过后才能改 `done` 前缀**。"通过"硬约束清单(全部满足才允许改名):

```
☐ 任务单"实现者完工标准"列表 100% 打勾, 一项不打勾都不算完工
☐ 必跑测试命令跑过且**全部 PASS**(允许 skip 的用例必须在任务单里明标"skip 算通过"才算)
☐ 没有测试是"先注释掉 / 临时禁用 / 跳过 fail" 这种姿态绕过
☐ 没有改动"不动"清单里的文件
☐ 偏差/缺工具已经上报用户并拍板, 没有自作主张
☐ 完成记录段已填(完成日期 / 改动摘要 / 测试结果 / 未覆盖风险 / 后续任务)
```

**禁令**:

- ✗ "测试我跑了能过, 我先改 done 前缀再 commit"  —— 测试结果必须落在完成记录里**和** commit 一起进库
- ✗ "测试有 1 个 fail, 但跟我这次改动无关,先改 done"  —— fail 就是 fail, 要么修, 要么上报用户拍 skip
- ✗ "我先改 done, 后面再补完成记录"  —— 完成记录是 done 改名的前置, 不是补丁
...
```
一次完整功能 commit 必含:
  ① 实现的代码文件(<业务层>/*.py / <ORM>/<models>.py / <工具包>/*.py / ...)
...
### 5.6 ADR / spec 落定前必读代码(影响清单不是猜出来的)

**触发场景**:
- 写新 ADR(`docs/adr/NNNN-xxx.md`)
- 改已有 spec.md(`test/<module>/spec.md`)
- 写新任务单(`task/waiting_NN_xxx.md`)

**硬约束**:

1. 落 ADR / spec 文件**之前**, Claude 必须显式 Read 以下文件:
   - ADR §影响清单 / spec §工具清单 里列出的**每个**目标文件(现存的)
   - 跟决策相关的现行实现(被取代的旧代码 / 配合调用方 / 同层兄弟模块)
   - 涉及到的 ORM 模型 / 状态枚举定义

2. 读完后, **基于代码现状**填:
   - ADR §影响清单: 写"现在这文件长什么样 → 本决策要改成什么样", 不写"大概要改一下 XX"
   - spec §工具清单: 标"已有(指向当前路径) / 旧代码搬(标待改名) / 新造(标位置)"——
     这三类判定的前提是已经看过代码确认了
   - 任务单 §改动文件清单 / §实现轮廓: 函数签名、类方法名、文件路径与代码现状一致

3. 责任划分:
   - 影响清单 / 工具清单 / 任务单实现轮廓的**正确性归需求窗口**
   - 实现窗口拿到任务单后**不再回头校对 ADR/spec 是否跟代码冲突**, 只负责按清单实现
   - 实现窗口发现清单跟代码冲突 → **停下来反问需求窗口**, 不自己改清单也不硬干

**反禁项**:

- ✗ 凭印象 / 凭过往聊天 / 凭 git log 填影响清单(必须 Read 当前代码)
- ✗ ADR §影响清单 写"改 `<path/to/some_module.py>` 一些方法"(没具体到哪些方法、改前长啥样)
- ✗ spec §工具清单 标"新造 `<SomeClass>.<some_method>()`" 但 `<SomeClass>` 已经有这方法
  (没读对应源文件就拍, 漏看了)
- ✗ 任务单 §实现轮廓 写的函数签名 / 类方法名跟代码现状对不上
- ✗ 用"实现窗口去对照一下代码就知道了"当借口跳过本节

**与 §4.3 工具实现顺序的衔接**:

- 本节(§5.6)保证**需求窗口拍板前**就识别出"缺哪些工具"
- §4.3 保证**实现窗口拿到任务单后**先把缺的工具造完再写业务
- 两步配合, 不应该出现"实现窗口写到一半才发现缺工具"

**例外**:

- 纯增量加字段 / 加常量 / 文档改动 等不依赖现行业务逻辑的改动, 可以不预读代码
- 但如果不确定"算不算纯增量", 默认走完整流程(读)
