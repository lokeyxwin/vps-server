...
- `test/ssh_worker/spec.md` v4 §3 路线 B ④（stage='connectable' 入库 + xray_version=""）
...
### 新建 `test/_data_structures/test_vps_record_v4.py`
...
⚠️ test/test_vps*.py / test/test_ip_register.py 等旧单测会失败
  ...
  仅需保证新 test/_data_structures/test_vps_record_v4.py 全过
...
测试文件: `test/_data_structures/test_vps_record_v4.py`
...
- [ ] 8 个测试全过(uv run python -m unittest test._data_structures.test_vps_record_v4)
...
新类、函数、方法、常量。新测试文件 `test/_data_structures/test_vps_record_v4.py`
...
3. 新建 `test/__init__.py`、`test/_data_structures/__init__.py`
   （unittest 包发现需要）

4. 新建 `test/_data_structures/test_vps_record_v4.py`
   - 8 个 TC 全过（`uv run python -m unittest ... -v` 输出 `OK`）
   - 顶部 / 尾部按 `test/README.md` 三段约定写
...
```
□ 跑 test/_data_structures/test_vps_record_v4.py 全过
