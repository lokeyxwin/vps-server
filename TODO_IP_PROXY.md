...
## 4.3 后续待办：rgvps 兼容已有 config（**rgIP 跑通后回头做**）

**场景**：真实生产服务器上 xray config 可能**已经有 client-* inbound 但没有 default-direct**
（之前的运维方式 / 别的脚本部署的）。当前 rgvps 流程：

- `is_config_blank` 为 True → write_default_config 覆盖（OK）
- `is_config_blank` 为 False → **保留原样不动**——结果就是后续内部 ping 18440 失败
  （因为根本没监听 18440），但 rgvps 还是会成功落库 xray_status=running

**期望**：rgvps 流程里如果 config 不空但**没有 default-direct inbound** →
**用 add 的方式补一条 default-direct + freedom outbound + routing 规则**，
不破坏已有的 client-* inbound。

**实现思路**：

新增 `xray.config.add_default_direct_to_config(current) -> dict` 纯函数
（add_proxy_binding 的姊妹版）：
- inbound: `_DEFAULT_DIRECT_INBOUND`（tag=default-direct）
- outbound: `_DEFAULT_DIRECT_OUTBOUND`（tag=direct）
- routing: default-direct → direct
- 如果 current 已有同名 tag → 幂等 noop 返回
- 不可变（deepcopy）

`services/vps_init.py` 改：
```python
if xray.config.is_config_blank(client):
    xray.config.write_default_config(client)
else:
    current = xray.config.read_config(client)
    if not _has_default_direct(current):
        new_config = xray.config.add_default_direct_to_config(current)
        xray.config.upload_config(client, new_config)
        xray.config.validate_config(client)
        xray.service.reload(client)
```

**时机**：rgIP 业务跑通真机后回头改（顺序见下方 Step 表新增 Step 9）。
...
| 9 | rgvps 改用 add 风格补 default-direct（不再覆盖式 build_vps_direct_config） | 见 4.3，Step 8 跑完回头做 |
| 10 | 巡检模块（is_active 维护）：暂不做 | |
